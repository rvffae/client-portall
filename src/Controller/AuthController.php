...
class GoogleCalendarController extends AbstractController
{
    ...
    public function callback(Request $request): Response
    {
        ...
        if (!$code) {
            ...
            return $this->redirectToRoute('dashboard'); // Changé de 'app_dashboard' à 'dashboard'
        }
...
// OU créer une route séparée pour le callback Google :

// Ajoute ce contrôleur séparé ou cette route dans un autre contrôleur
class AuthController extends AbstractController
{
    #[Route('/auth/google/callback', name: 'google_auth_callback')]
    public function googleCallback(Request $request, GoogleCalendarService $calendarService): Response
    {
        $code = $request->query->get('code');
        ...
        if ($calendarService->authenticate($code)) {
            $this->addFlash('success', 'Google Calendar connecté avec succès !');
        } else {
            $this->addFlash('error', 'Erreur lors de la connexion à Google Calendar');
        }
